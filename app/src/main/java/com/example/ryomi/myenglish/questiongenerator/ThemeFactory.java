...
public class ThemeFactory {
    public static Theme createTheme(ThemeData themeData){
        ...
        if (themeID.equals(ThemeMappings.NAME_went_to_SCHOOL_from_START_to_END)){
            return new NAME_went_to_SCHOOL_from_START_to_END(
                    new WikiDataSPARQLConnector(WikiBaseEndpointConnector.JAPANESE),
                    themeData);
        } else if (themeID.equals(ThemeMappings.NAME_possessive_blood_type_is_BLOODTYPE)){
            return new NAME_possessive_blood_type_is_BLOODTYPE(
                    new WikiDataSPARQLConnector(WikiBaseEndpointConnector.JAPANESE),
                    themeData);
        } else if (themeID.equals(ThemeMappings.NAME_plays_SPORT)){
            return new NAME_plays_SPORT(
                    new WikiDataSPARQLConnector(WikiBaseEndpointConnector.JAPANESE),
                    themeData);
        } else if (themeID.equals(ThemeMappings.NAME_went_to_SCHOOL_So_did_NAME2)){
