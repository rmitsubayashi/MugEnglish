...
import com.example.ryomi.mugenglish.gui.widgets.LessonListAdapter;
...
public class LessonList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        appBar = (Toolbar)findViewById(R.id.lesson_list_tool_bar);
        setSupportActionBar(appBar);

        listView = (RecyclerView) findViewById(R.id.lesson_list_list);
        listView.setLayoutManager(new LinearLayoutManager(this));
        loading = (ProgressBar) findViewById(R.id.lesson_list_loading);

        populateNavigationDrawer();

        // Initializing Drawer Layout and ActionBarToggle
        DrawerLayout drawerLayout = (DrawerLayout) findViewById(R.id.activity_lesson_list);
        ActionBarDrawerToggle actionBarDrawerToggle = new ActionBarDrawerToggle(
                this,drawerLayout,appBar,R.string.lesson_list_navigation_drawer_open, R.string.lesson_list_navigation_drawer_close){
        ...
        BottomNavigationView nav = (BottomNavigationView)findViewById(R.id.lesson_list_bottom_navigation_view);
        GUIUtils.prepareBottomNavigationView(this, nav);
    }

    private void populateThemes(LessonCategory category){
        if (firebaseAdapter != null)
            firebaseAdapter.cleanup();
        loading.setVisibility(View.VISIBLE);
        FirebaseDatabase db = FirebaseDatabase.getInstance();
        DatabaseReference ref = db.getReference(FirebaseDBHeaders.LESSONS);
        Query query = ref.orderByChild(FirebaseDBHeaders.LESSON_CATEGORIES).equalTo(category.getIndex());
        firebaseAdapter = new LessonListAdapter(query, loading);
    ...
    private void populateNavigationDrawer(){
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                List<LessonCategory> categories = new ArrayList<>();
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
                    LessonCategory category = snapshot.getValue(LessonCategory.class);
                ...
                final DrawerLayout drawerLayout = (DrawerLayout) findViewById((R.id.activity_lesson_list));
                NavigationView navigationDrawer = (NavigationView) findViewById(R.id.lesson_list_navigation_drawer);
                Menu navigationDrawerMenu = navigationDrawer.getMenu();

                for (final LessonCategory category :categories){
    ...
    private class ThemeCategoryIndexComparator implements Comparator<LessonCategory> {
        @Override
        public int compare(LessonCategory category1, LessonCategory category2) {
