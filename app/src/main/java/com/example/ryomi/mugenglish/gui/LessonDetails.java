...
import com.example.ryomi.mugenglish.gui.widgets.LessonDetailsAdapter;
import com.example.ryomi.mugenglish.questiongenerator.Lesson;
import com.example.ryomi.mugenglish.questiongenerator.LessonFactory;
...
public class LessonDetails extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra("lessonData") && intent.hasExtra("backgroundColor")) {
            //get data
            lessonData = (LessonData) intent.getSerializableExtra("lessonData");
            appBar = (Toolbar) findViewById(R.id.lesson_details_tool_bar);
            setSupportActionBar(appBar);
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);


            list = (RecyclerView) findViewById(R.id.lesson_details_instanceList);
            noItemTextView = (TextView) findViewById(R.id.lesson_details_no_items);
            loading = (ProgressBar) findViewById(R.id.lesson_details_loading);
    ...
    public boolean onContextItemSelected(MenuItem item) {
        LessonInstanceData data = ((LessonDetailsAdapter)list.getAdapter()).getLongClickPosition();
    ...
    private void populateData(){
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
            ...
            DatabaseReference ref2 = FirebaseDatabase.getInstance().getReference(
                    FirebaseDBHeaders.LESSON_INSTANCES + "/"+userID+"/"+ lessonData.getId());
            list.setLayoutManager(new LinearLayoutManager(this));
            firebaseAdapter = new LessonDetailsAdapter(ref2, noItemTextView, loading);
    ...
    private void setThemeColor(int color){
        ...
        findViewById(R.id.activity_lesson_details).setBackgroundColor(color);
    ...
    private void adjustLayout(){
        ...
        SharedPreferences sharedPreferences =
                PreferenceManager.getDefaultSharedPreferences(LessonDetails.this.getApplicationContext());
    ...
    private void addActionListeners(){
        createButton = (FloatingActionButton)findViewById(R.id.lesson_details_add);
    ...
    private void createNewInstance(){
        ...
        //load theme class
        Lesson theme = LessonFactory.parseLesson(lessonData);
    ...
    private void removeInstance(LessonInstanceData data){
        if (FirebaseAuth.getInstance().getCurrentUser() == null)
            return;

        FirebaseDatabase db = FirebaseDatabase.getInstance();
        String userID = FirebaseAuth.getInstance().getCurrentUser().getUid();
        DatabaseReference ref = db.getReference(FirebaseDBHeaders.LESSON_INSTANCES + "/" +
