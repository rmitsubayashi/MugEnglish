...
public abstract class Lesson {
	...
	protected LessonData lessonData;
	...
	public Lesson(WikiBaseEndpointConnector connector, LessonData data){
	...
	private void populateUserQuestionHistory(){
		if (FirebaseAuth.getInstance().getCurrentUser() != null) {
			...
			ref.addListenerForSingleValueEvent(new ValueEventListener() {
				...
				public void onDataChange(DataSnapshot dataSnapshot) {
					for (DataSnapshot child : dataSnapshot.getChildren()) {
						LessonInstanceData data = child.getValue(LessonInstanceData.class);
	...
	private void populateExistingNewQuestions(final Set<WikiDataEntryData> interests){
		...
		DatabaseReference ref = db.getReference(
				FirebaseDBHeaders.QUESTION_TOPICS + "/" + lessonData.getId());
		ref.addListenerForSingleValueEvent(new ValueEventListener() {
			@Override
	...
	private void saveInstance(){
		...
		LessonInstanceData data = new LessonInstanceData(key, lessonData.getId(),
				userID, questionSets, topicList, System.currentTimeMillis(),0);
		DatabaseReference ref2 = db.getReference(
				FirebaseDBHeaders.LESSON_INSTANCES + "/" + userID + "/" + lessonData.getId() + "/" +key);
		ref2.setValue(data);
