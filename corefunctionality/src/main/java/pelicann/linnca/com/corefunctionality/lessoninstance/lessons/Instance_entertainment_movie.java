...
public class Instance_entertainment_movie extends LessonInstanceGenerator {
    public Instance_entertainment_movie(){
        super();
        this.lessonKey = Entertainment_movie.KEY;
    ...
    protected synchronized void processResultsIntoEntityPropertyData(Document document){
        ...
        for (int i=0; i<resultLength; i++){
            ...
            String movieEN = SPARQLDocumentParserHelper.findValueByNodeName(head, "movieENLabel");
            String movieJP = SPARQLDocumentParserHelper.findValueByNodeName(head, "movieLabel");
            Translation movieTranslation = new Translation(movieEN, movieJP);
            List<Translation> properties = new ArrayList<>();
            properties.add(personTranslation);
            properties.add(movieTranslation);
