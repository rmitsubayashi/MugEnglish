...
public abstract class LessonListViewer {
    protected final List<List<LessonListRow>> lessonLevels = new ArrayList<>();
    //so we can go people1 -> people2 -> people3
    protected Map<String, Integer> titleCount;
    ...
    public void debugUnlockAllLessons(Database db){
        db.clearAllLessons(lessonLevels);
    }

    protected String getNextReviewID(){
        String id = LessonData.formatReviewID(reviewIDLevel, reviewIDNumber);
        reviewIDNumber++;
        return id;
    }

    protected void nextLevelResetReviewIDCt(){
        reviewIDNumber = 0;
        reviewIDLevel++;
    }

    protected void adjustRowTitles(List<LessonListRow> rows){
